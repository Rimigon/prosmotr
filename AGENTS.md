...
> Последнее обновление: 2026-09-10 20:34:48 UTC
...
#### Перемотка на open-GOP H.264 (артефакты после seek) — avformat-демуксер

Некоторые рипы (SiteRIP и т.п.) — **open-GOP H.264**: в mp4-боксе `stss` (sync sample box)
помечены non-IDR recovery-кадры (SEI recovery point) как «ключевые», а настоящие IDR-кадры
(они в потоке есть, но в `stss` не внесены) игнорируются. Встроенный mp4-демуксер VLC доверяет
`stss` → при перемотке садится на non-IDR кадр → декодер показывает мусор/артефакты, пока не
дойдёт до настоящего IDR (до ~8 c — «иногда остаются надолго»). ffmpeg-демуксер ищет реальные
IDR и делает чистый seek (проверено: ffmpeg CLI `-ss` на таком файле даёт чистые кадры).

**Решение — форсировать ffmpeg-демуксер для MP4/MOV/M4V:**
`VideoPlaybackService.Load` добавляет `:demux=avformat` для `.mp4/.m4v/.mov` (mkv/avi/webm —
встроенные демуксеры VLC, они там надёжнее: главы, субтитры).

Сопутствующие правки перемотки (итог долгой отладки на 4K open-GOP):

- **`--input-fast-seek` убран** из `LibVlcProvider.VlcOptions` — на длинном GOP замедлял догон
  в разы и делал seek неточным (Screenbox и др. работают без него).
- **Аппаратное декодирование выключено** (`EnableHardwareDecoding = false`): DXVA2/D3D11VA на
  open-GOP дают артефакты/зелёный при seek (баг AMD, LAVFilters issue #580); софтверный ffmpeg
  обрабатывает recovery point корректно. 4K 25fps CPU тянет (обычный просмотр не рвётся).
- **Стрелки ←/→**: одиночное нажатие = один `SeekTo` (как клик таймлайна); удержание =
  накопление смещения (`_pendingStepMs`) + таймер ~80 мс шлёт seek на последнюю позицию.
  Старый `_stepThrottle`/`_pendingStepCount` (накопление направлений по 80 мс) заменён.

Диагностика: файл без IDR в `stss` можно найти ffprobe: `ffprobe -show_entries packet=flags` —
ключевые показываются `K__` (не `K_I`), при этом NAL-анализ (`-v debug`, `nal_unit_type: 5`)
показывает реальные IDR. Не пугаться «K__» — это не значит, что IDR нет.
...
- **⚠️ Клон возвращается после перезагрузки/сна (persistence database).** `SDC_TOPOLOGY_CLONE|SDC_APPLY` (EnableClone) **записывает** клон в persistence database Windows как последнюю применённую топологию (документированное поведение SDC_TOPOLOGY_XXX: «set last display path settings»). Основной путь `RestoreExtend` (`SDC_USE_SUPPLIED_DISPLAY_CONFIG|SDC_APPLY` без `SDC_SAVE_TO_DATABASE`) применяет extend **только временно** — по таблице сценариев MSDN это «temporary display configuration (will not be saved)». Итог: после «выключения» дублирования в базе всё ещё лежит клон, и после перезагрузки/сна Windows восстанавливает клон, даже когда приложение не запущено. **Фикс:** в основном пути `RestoreExtend` добавлен `SDC_SAVE_TO_DATABASE` (0x200; валиден только с `SDC_USE_SUPPLIED_DISPLAY_CONFIG`) — extend теперь применяется И сохраняется в базу. Не удаляй этот флаг. Fallback (`SDC_TOPOLOGY_EXTEND`) пишет в базу сам по себе (семантика SDC_TOPOLOGY_XXX), поэтому трогать его не нужно. Остаточные случаи: процесс убит в режиме клона (см. «Восстановление при shutdown») и пробуждение из S3 на некоторых драйверах GPU, которые восстанавливают состояние не из базы (KB 2890797 — подтверждённый баг Microsoft уровня «revert after resume»).
...
### 5.34. Запоминание скорости и аудиодорожки для конкретного файла

- **Что хранится:** `PlaybackPosition` (`positions.json`) кроме позиции держит `Rate` и
  `AudioTrackId`/`AudioTrackName` — выбор скорости и аудиодорожки отдельно для каждого видео.
- **Настройки:** `RememberRatePerFile` и `RememberAudioTrackPerFile` (`AppSettings`, дефолты
  **true** — обе включены из коробки; тумблеры в окне настроек, раздел «Видео»).
- **Когда сохраняется:** в `SavePosition()` (пауза/стоп/seek/закрытие) и сразу при изменении
  скорости (`ApplyRate`) / выборе дорожки (`SelectAudioTrack`). Сохраняется только реальная
  дорожка (`CurrentAudioTrack > 0`); `-1/0` (дефолт) не пишется.
- **Когда применяется:** скорость — в `BeginPlayback` (из stored перед `LoadAndPlayDeferred`);
  аудиодорожка — **в `OnPlaying`**, т.к. список дорожек LibVLC отдаёт только после старта
  воспроизведения. Поле `_pendingAudioTrackId`/`_pendingAudioTrackName` ставится в `BeginPlayback`
  и актуализируется в `SelectAudioTrack` — повторный `OnPlaying` (replay из `EndReached`
  перезагружает дорожку и сбрасывает выбор) применяет актуальную дорожку, а не устаревшую.
- **Применение параметров в `OnPlaying` — только при реальном изменении.** Событие `Playing`
  приходит и на каждый resume после паузы; безусловный `SetRate`/`SetVolume`/`SetMute` при каждой
  пауза→play перезапускал аудиовыход LibVLC — на Bluetooth (A2DP) звук пропадал на секунду после
  возобновления (особенно при скорости ≠ 1×, когда активен ресемплер). Теперь применяется только
  если значение отличается от текущего у плеера (`Math.Abs(_playback.Rate - _pendingRate) > 0.001f`
  и т.п.). Свежая загрузка/реплей сбрасывают Rate/Volume в дефолты — guard пропускает применение.
- **Сопоставление при восстановлении:** сначала по `AudioTrackId`; если id сместился (файл
  пересобран/демукшен) — запасной поиск по `AudioTrackName` (case-insensitive). Не найдено —
  остаётся дефолтная дорожка (мягкая деградация).
- **`TrackDescription` в LibVLCSharp не nullable-аннотирован** — нельзя писать
  `Array.Find(...)?.Name` / `== null` (ошибки CS0023/CS0019). Поиск — через `foreach`
  (`FindAudioTrackName`/`MatchAudioTrack`).
- **Удаление файла** сбрасывает запись (`_positions.Remove`) — скорость/дорожка удалённого
  файла не воскресают.

### 5.35. Озвучка на уровне папки (сезон сериала)

Запоминание аудиодорожки **для папки**: выбор озвучки в одной серии становится дефолтом папки и
применяется ко всем её файлам при открытии (если такая дорожка у файла есть). Решения пользователя:
папка-сезон (подпапки не затронуты), дефолт папки **важнее** пер-файловой памяти, выбор
«По умолчанию» (дорожка 0) **сбрасывает** дефолт папки.

- **Отдельное хранилище — `FolderAudioTrackStore`** (`IFolderAudioTrackStore`), файл
  `%LOCALAPPDATA%\Prosmotr\folder-audio-tracks.json` (`Dictionary<string, FolderAudioTrack>`,
  ключ `ToLowerInvariant()`, атомарная запись, debounce 1.5 с). НЕ в positions.json: он — плоский
  `Dictionary<string, PlaybackPosition>`, и добавление туда папок сломало бы формат и тесты
  round-trip. Тот же паттерн, что `PlaybackPositionStore` (переопределяемый `directory` для тестов,
  `Flush()` при выходе из приложения через `App.OnExit`).
- **Применение в `BeginPlayback`:** приоритет — дефолт папки (`Item.DirectoryPath`, если
  `RememberAudioTrackPerFolder`) → пер-файловая память → дорожка по умолчанию. Результат идёт в
  существующие `_pendingAudioTrackId/_pendingAudioTrackName`; `OnPlaying`/`MatchAudioTrack`
  не менялись (id → имя с мягким фолбэком).
- **Запись в `SelectAudioTrack`:** id > 0 → `_folderTracks.Set(DirectoryPath, id, name)`;
  id == 0 («По умолчанию») → `_folderTracks.Clear(DirectoryPath)`. Пер-файловая `SavePosition()`
  осталась как есть — обе памяти пишутся, при чтении дефолт папки побеждает.
- **Настройка:** `RememberAudioTrackPerFolder` (default **true**), тумблер «Запоминать озвучку для
  папки (сериала)» в окне настроек, раздел «Видео». При выключении папки игнорируются, записи не
  удаляются.
- **Крайние случаи:** удаление файла не трогает дефолт папки (другие серии живы); файл переехал —
  применится дефолт новой папки; дорожки нет в файле — мягкий фолбэк на дефолтную.
...
- **Fast-resume НЕ терять (иначе перекачка всего файла):** автосохранение MonoTorrent срабатывает
  только при успешном переходе в Stopped, а `StopAsync(2s)` с таймаутом может прервать остановку →
  битфилд терялся, при повторном открытии движок качал ВЕСЬ файл заново (bitfield 0% при полном
  файле на диске → чтения блокируются → зависание). Поэтому: явный `SaveFastResumeAsync()` в
  `CloseSessionAsync` ДО остановки + периодическое сохранение в таймере прогресса (каждые 10 с
  и сразу при 100%).
- **МАГНЕТЫ: MonoTorrent НЕ загружает fast-resume вообще.** `MaybeLoadFastResumeAsync` вызывается
  только из `AddAsync`, а для магнета в этот момент `!HasMetadata` → ранний выход; после прихода
  метаданных загрузка не повторяется → при каждом открытии той же ссылки bitfield пустой →
  перекачка всего файла (симптом: `downloaded=0%` при полном файле на диске, фризы, прыжки).
  Фикс: `LoadSavedFastResumeAsync` в `InitSessionAsync` после метаданных — `StopAsync(5s)` →
  `manager.LoadFastResumeAsync(FastResume.TryLoad(...))` (требует Stopped) → `StartAsync()`.
  Поток ещё не создан, перезапуск безопасен.
- **`_player.Time = X` (libvlc set_time) может БЛОКИРОВАТЬ вызывающий поток**, если входной поток
  застрял на недокачанном куске → UI замрёт без индикаторов. Resume-seek выполняется через
  `Task.Run` с таймаутом 5 с (timeout → пропуск resume), НЕ на UI-потоке.
...
- **Лента миниатюр и боковые стрелки окна НЕ показываются на экране магнет-стриминга.**
  `MainViewModel.ShowThumbnailStrip` и `ShowWindowNavArrows` явно исключают `TorrentStreamViewModel`
  (оба поднимаются в `OnCurrentContentChanged`). Иначе после «На главную» → торрент оставалась
  лента миниатюр из прежней папки (файлы к торренту отношения не имеют), а клик по стрелке уводил бы
  с торрент-сессии на файл из этой папки.
- **Кнопка «На главную» — только в верхней панели окна** (`Home24` → `GoHomeCommand`). В торрент-
  просмотрщике её НЕ дублируем: верхний тулбар виден и в фазах загрузки/просмотра (кроме полного
  экрана), а лишний домик в углу экрана раздражает. В полном экране выход — по Esc (`CloseSessionCommand`).
  `GoHome` выходит из полного экрана, закрывает PiP, для торрента идёт через `CloseSessionAsync`,
  иначе ставит `EmptyStateViewModel`. **Список галереи НЕ очищается** — навигация стрелками
  возвращает к просмотру с того же файла (в трее ленты осознанный компромисс).
- **`DisposeContentAfterSwap(old, next)`** — выделено из `UpdateCurrentContent` (отложенный
  Background-dispose старого контента + закрытие торрент-сессии после освобождения плеера).
  `GoHome` использует тот же путь, не дублируя логику.
- **Кнопка магнет-стриминга есть в верхней панели** (`Link24` → `OpenMagnetCommand`), поэтому она
  доступна с любого экрана (фото/видео/пустой), а не только со стартового. На самом торрент-плеере
  кнопка недоступна (`CanOpenMagnet` = `CurrentContent is not TorrentStreamViewModel`) — иначе
  повторный ввод дал бы «уже есть активная сессия». `RefreshCommandStates`/`OnCurrentContentChanged`
  пересчитывают доступность.
- **Пауза: звук не должен пропадать.** `TogglePlayPause` снимает паузу через `SetPause(false)`, а не
  `Play()` (Play() на паузе пересоздаёт аудиовыход LibVLC → секунда тишины). Параметры
  скорость/громкость/озвучка применяются в `OnPlaying` строго один раз на загрузку дорожки
  (`_paramsApplied`, сбрасывается в `CreatePlayer`): `OnPlaying` приходит и на каждое снятие паузы,
  а любой `SetRate`/`SetVolume`/`SetAudioTrack` тоже пересоздаёт аудиовыход.
- **Статистика страницы загрузки:** `TorrentSession.SeedsCount` (`manager.Peers.Seeds`) и `ElapsedSeconds`
  обновляются в таймере движка (раз в секунду). UI показывает «Скачано (% · X из Y)», скорость ↓/↑,
  пиры, сиды, прошло, осталось; угловая плашка на видео — три строки (размер, скорость/пиры/сиды, ETA).

#### DHT: почему без фикса магнет «висит» и качает единицы КБ/с

DHT — основной источник пиров для магнета (у голой magnet-ссылки нет трекеров). Если DHT не поднялась,
монолитный симптом — «Поиск пиров…» без конца или единицы КБ/с: остаются только те пиры, что нашлись
иначе. Три причины, все воспроизведены и закрыты:

1. **У MonoTorrent один зашитый bootstrap-хост** — `router.bittorrent.com`
   (`MonoTorrent.Dht.Tasks.InitialiseTask.GenerateBootstrapNodes`). Он сейчас **не отвечает** вообще
   (проверено UDP-пингом DHT-запросом), т.е. «из коробки» DHT не поднимается никогда. Лечение:
   `Infrastructure`-класс `Services/Torrent/DhtBootstrap.cs` резолвит набор публичных хостов
   (transmissionbt, libtorrent, aelitis, bt.am) **и добавляет проверенные статические IP**, после чего
   `TorrentEngineService` создаёт `DhtEngine` сам и отдаёт его через
   `Factories.Default.WithDhtCreator(() => dht)`. `DhtEngine.Add(...)` в состоянии NotReady кладёт узлы
   в `PendingNodes`, а при непустом списке MonoTorrent вообще не трогает свой встроенный хост.
2. **Один живой bootstrap-узел недостаточен.** Проверено: 1 узел → 0 узлов в таблице и через 20 с
   (вечное `Initialising`), 3 живых → 45–58 узлов за 2 с. Поэтому статика — основа, DNS — добор.
   Список статических IP со временем устаревает: при жалобах на скорость перепроверять UDP-пингом
   (DHT-запрос `d1:ad2:id20:…e1:q4:ping1:t2:aa1:y1:qe`); в `DhtBootstrap` держать только живые.
3. **Кэш метаданных магнета ломает DHT — самое неочевидное.** При `AutoSaveLoadMagnetLinkMetadata = true`
   (дефолт) и наличии файла `torrents\.cache\metadata\<hash>.torrent` MonoTorrent создаёт менеджер уже
   с торрентом (`ClientEngine.AddAsync`: `Torrent.TryLoad(...) → manager.SetMetadata(torrent)`), и в этом
   пути DHT **не инициализируется вообще** — вечное `Initialising`, 0 узлов, пиры только с трекеров.
   Воспроизведено: с файлом — 0 узлов и 0 пиров, без файла — 58 узлов за 2 с и сотни пиров.
   Поэтому в `TorrentEngineService.GetEngine` стоит **`AutoSaveLoadMagnetLinkMetadata = false`**: метаданные
   каждый раз берутся от пиров (секунды), зато DHT живая. Возврат к `true` вернёт дефект.

Диагностика (в логе, префикс `[Torrent]`): `DHT bootstrap nodes: N (static M)` — сколько узлов подложили;
`metadata wait Ns: dht=… nodes=… peers=…/…/…` — watchdog, пишется только если метаданные вовсю ждутся
(обычно их не видно); `Ready: DHT nodes=N, known peers=N`. В UI на экране загрузки есть строка **DHT**
(`нет узлов` = пиры только через трекеры).
